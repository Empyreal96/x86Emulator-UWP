...
namespace x86Emulator.CPU
{
    public partial class CPU
    {
        ...
        private uint DisassemblerRead(uint offset, int size)
        {
            ...
            {
                uint ret;
                bool log = Helpers.DebugLog;

                Helpers.DebugLog = false;
                ret = Memory.Read(CurrentAddr + offset, size);
                Helpers.DebugLog = log;
        ...
        {
            if (Logging && Helpers.DebugFile)
            {
                Helpers.Logger(String.Format("AX {0:X4} BX {1:X4} CX {2:X4} DX {3:X4}", AX, BX, CX, DX));
                Helpers.Logger(String.Format("SI {0:X4} DI {1:X4} SP {2:X4} BP {3:X4}", SI, DI, SP, BP));
                Helpers.Logger(String.Format("CS {0:X4} DS {1:X4} ES {2:X4} SS {3:X4}", CS, DS, ES, SS));
            }   
        }
        ...
        public void Cycle(bool logging)
        {
            ...
            if (logging && Helpers.DebugFile)
                Helpers.Logger(String.Format("{0:X}:{1:X} {2}", CS, EIP, disasm.InstructionText));
